...
<div class="create-panel panel" style="display: none;">
    ...
    <button class="extra-wheel btn-cr">Extra</button>
    <button class="generate-wheel btn-cr">Generate</button>
    <div class="wheel-extra" style="display: none;">
        <span>url for wheel's cap image</span>
        <input class="cap-url"></input>
        <br>
        <span>image size in percentage(0-100)</span>
        <input class="cap-size" type="number" value="100" min="0" max="100"></input>
    </div>
...
OBR.onReady(() => {
    ...
    const extra = document.getElementsByClassName('extra-wheel')[0]
    ...
    extra.addEventListener("click", () => {
        let wheelExtra = document.getElementsByClassName('wheel-extra')[0];
        if (wheelExtra.style.display == "block")
            wheelExtra.style.display = "none";
        else
            wheelExtra.style.display = "block";
    });
    ...
    edit.addEventListener("click", () => {
        if (select.value != ''){
            ...
            let wheel = JSON.parse(localStorage.getItem(select.value));
            ...
            */
            if (wheel !== undefined){
                let extraPanel = document.getElementsByClassName('wheel-extra')[0].children;
                extraPanel[1].value = wheel[0]["url"];
                extraPanel[4].value = wheel[0]["size"];
            }
    ...
    del.addEventListener("click", () => {
        ...
        if (sure_panel.style.display == "grid")
    ...
    gen.addEventListener("click", () => {
        ...
        const extraPanel = document.getElementsByClassName('wheel-extra')[0]
        if(counter.value != 0 && newName.value != ''){
            ...
            const p = extraPanel.children;
            if (p[1].value != "")
                str += '\n\t],\n\t"wheel":[\n' + JSON.stringify({url: p[1].value, size: p[4].value});
            str += '\n\t]\n}'
